...
import nu.mine.wberg.listenerapp.analysis.mfcc.bmfcc.math.Matrix;
...
public class MFCC
{
 ...
 private Matrix getMelFilterBanks()
   ...
   * @throws IllegalArgumentException raised if mehtod contract is violated
   */
  public double[][] process(double[] input) throws IllegalArgumentException
  {
    //check for null
    if(input == null)
      throw new IllegalArgumentException("input data must not be a null value");

    // truncate to multiple of hopSize
    if ((input.length % hopSize) != 0) {
        input = Arrays.copyOfRange(input, 0, (input.length / hopSize) * hopSize);
    }
